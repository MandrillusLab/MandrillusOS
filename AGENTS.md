...
> qualquer IA usada em modo chat/planejamento (Codex, Copilot Chat etc.).
...
Fase 1 fechada. **Issue #8 (shell Drill)** concluída — implementação completa, incluindo a correção pós-fechamento do bug de `Array.Copy` (ver [docs/context/constraints.md](docs/context/constraints.md#korlib)). **Issue #9 (timer PIT)** também fechada — 250Hz, `SystemTimer` + comando `uptime`, testado no QEMU e mesclado em `master`. Continua sendo digitado manualmente, linha a linha — uma IA não deve gerar o arquivo inteiro, só discutir trechos.
...
- [docs/context/os-landscape.md](docs/context/os-landscape.md) -  referências primárias (OSDev, MOSA, Cosmos) e ranking dos 20 projetos de SO em C# comparáveis, organizados por linhagem de toolchain (MOSA/Cosmos/NativeAOT).
