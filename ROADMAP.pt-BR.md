...
> **Nota:** uma investigação confirmou (via [documentação do MOSA](https://www.mosa-project.org/a-dive-into-baremetal.html), o log de boot real e inspeção direta de `Source/Mosa.DeviceDriver/ISA/` no repositório do MOSA) que toda a infraestrutura de baixo nível da Fase 1 (GDT, IDT, gerenciamento de memória, teclado, vídeo) já é fornecida pelo kernel MOSA BareMetal sobre o qual o `Mandrillus.Kernel.x86` é construído. Os itens abaixo marcados como *(herdado)* não são trabalho autoral. A única exceção é o shell interativo (#8) — um componente de camada de aplicação, não infraestrutura de baixo nível — que é o entregável autoral do Mandrillus para esta fase.
...
- **Resumo da investigação de PIT/timer (Fase 2, Issue #9):**
  ...
  > ⚠️ Conduzida sobre a `master` do MOSA, não sobre a release `2.6.1.1669` à qual este projeto fixa seus pacotes — veja a ressalva de versão sob a Fase 2 acima.
