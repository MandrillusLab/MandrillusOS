...
- **Fonte do Singularity RDK 2.0 (resgate não-oficial do CodePlex, mirror no GitHub)** — <https://github.com/TeddyAlbina/Singularity-RDK> (também espelhado em <https://github.com/dz333n/Singularity-OS>) — o CodePlex foi encerrado; estes são resgates feitos pela comunidade do último release público, não um repositório oficial da Microsoft. Árvore completa de kernel/HAL/compilador Bartok/drivers por trás dos artigos acima. Útil para comparação direta do HAL x86 e do scheduler ao revisitar o Issue #9, e como estudo dos padrões de declaração de recursos de driver (atributos [IoPortRange(...)]) — não é adaptável em bloco, já que todo o modelo de SIP/canais (processos isolados por software, sem isolamento via MMU) é arquiteturalmente incompatível com um kernel monolítico simples como o do Mandrillus.
- **Importante: o Midori é uma reescrita do zero, não um fork do código-fonte do Singularity.** O Midori (2007–2015) herdou as ideias de design do Singularity — microkernel, isolamento por passagem de mensagens, código gerenciado em toda a pilha — e parte das mesmas pessoas, mas foi construído de forma independente em C# mais uma linguagem interna chamada M#, chegando a ~100 desenvolvedores e, em certo momento, alimentando o backend de busca por linguagem natural da Microsoft para a Costa Oeste e Ásia. Ele avançou mais em direção a computação distribuída/cloud (assincronia nativa, segurança por capacidades, x86/x86-64/ARM) — escopo bem além de um kernel bare-metal de máquina única. O código-fonte do Midori nunca foi liberado; tudo que é público sobre sua implementação real está em prosa (blog de Joe Duffy, palestras), não em código. O RDK do Singularity acima é, portanto, o único código-fonte real dessa linhagem disponível para leitura.
