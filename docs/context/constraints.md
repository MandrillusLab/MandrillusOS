...
- ~~Regressão do teclado no Hyper-V~~ — **resolvida separadamente** (não relacionada ao PIT — na época, testando sem `HardwareSetup.RegisterPitTimer()` ativo, o problema persistia mesmo assim, confirmando que não tinha relação com o PIT). Causa raiz e fix documentados na seção [Bug do compilador MOSA: eliminação incorreta de branch de saída em loop com condição && composta](#️-bug-do-compilador-mosa-eliminação-incorreta-de-branch-de-saída-em-loop-com-condição--composta) acima e em [tooling.md](tooling.md#emuladores).
...
3. **`ConvertU64ToR8`/`ConvertI64ToR8` ausente/truncado em x86** (ver seção acima). O mais forte dos seis — vem com causa raiz precisa **e** evidência histórica de que a equipe já sabe como resolver (mesma técnica usada com sucesso na direção oposta, `R8ToI8`).
...
Todos os seis são candidatos mais fortes que um simples "não funciona" — já chegam com causa raiz identificada e reprodução documentada.
