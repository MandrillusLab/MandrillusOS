...
> **Note:** Investigation confirmed (via [MOSA docs](https://www.mosa-project.org/a-dive-into-baremetal.html), the actual boot log, and direct inspection of `Source/Mosa.DeviceDriver/ISA/` in the MOSA repository) that all of Phase 1's low-level infrastructure (GDT, IDT, memory management, keyboard, video) is already provided by the MOSA BareMetal kernel that `Mandrillus.Kernel.x86` builds on. Items below marked *(inherited)* are not original work. The one exception is the interactive shell (#8) — an application-layer component, not low-level infrastructure — which is Mandrillus's own original deliverable for this phase.
...
> **Note on the PIT timer:** deeper investigation (two independent AI-assisted research passes over the MOSA `master` source) confirmed there's no PIT/8253/8254 driver anywhere in MOSA — but also revealed IRQ0 is *not* idle: `Mosa.Kernel.BareMetal`'s `Scheduler` already consumes it (`Scheduler.ClockInterrupt`) at whatever default frequency the BIOS/QEMU left the PIT running at (~18.2Hz, unconfigured). `HAL.Sleep()` exists as an API but its body is an empty `// TODO`. So this isn't "add a timer that doesn't exist" — it's "take control of a timer that's already silently driving the scheduler." That's why it belongs here, as a Phase 2 prerequisite of real preemptive scheduling, rather than in Phase 1 (which has zero dependency on it — see Phase 1 note above).
>
> ⚠️ **Version caveat:** this investigation was conducted against the MOSA `master` branch, not against the `2.6.1.1669` release this project actually pins its packages to (see README's package version note). The conclusions above — particularly IRQ0's dispatch behavior and `HAL.Sleep()`'s empty body — have not been separately verified against the `2.6.1.1669` source and should be treated as `master`-only until a compatibility check is done. Before implementing Issue #9, re-verify these findings against the code the pinned package version actually ships (or explicitly re-confirm they still hold if/when the pin is updated).
>
...
- **PIT/timer research summary (Phase 2, Issue #9):**
  ...
  > ⚠️ Conducted against MOSA `master`, not the `2.6.1.1669` release this project pins to — see version caveat under Phase 2 above.
